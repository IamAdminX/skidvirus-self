...
class TokenBio:
    ...
    def send(self) -> None:
        ...
        result=self.session.patch(f"https://discord.com/api/v9/users/%40me/profile",json={
            'session_id': self.rand_str(32),
            'bio': self.instance.bioo,
        })

        if result.status_code == 200:
            print(f" {Fore.BLUE}<*> Successfully Changed Bio To {self.instance.bioo} {Fore.RED}({str(result.status_code)})")
        elif result.text.startswith('{"captcha_key"'):
            print(f" {Fore.RED}<!> Error Changing Bio To {self.instance.bioo} {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Captcha)")
        elif result.text.startswith('{"message": "401: Unauthorized'):
            print(f" {Fore.RED}<!> Error Changing Bio To {self.instance.bioo} {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Unauthorized)")
        elif "Cloudflare" in result.text:
            print(f" {Fore.RED}<!> Error Changing Bio To {self.instance.bioo} {Fore.BLUE}({str(result.status_code)}){Fore.RED} (CloudFlare Blocked)")
        elif "\"code\": 40007" in result.text:
            print(f" {Fore.RED}<!> Error Changing Bio To {self.instance.bioo} {Fore.BLUE}({str(result.status_code)}){Fore.RED} (User Banned)")
        elif "\"code\": 40002" in result.text:
            print(f" {Fore.RED}<!> Error Changing Bio To {self.instance.bioo} {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Locked Token)")
        elif "\"code\": 10006" in result.text:
            print(f" {Fore.RED}<!> Error Changing Bio To {self.instance.bioo} {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Invalid Invite)")
        else:
            print(f" {Fore.RED}<!> Error Changing Bio To {self.instance.bioo} {Fore.RED}({str(result.status_code)}){Fore.BLUE} ({result.text})")
...
def TokenBioSet():
    ...
    instances = []
    max_threads=5
    biooo = input(f"{Fore.RED} <~> Bio: {Fore.BLUE}")
