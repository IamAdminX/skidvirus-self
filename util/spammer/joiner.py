...
class Joiner:
    ...
    def join(self) -> None:
        ...
        if result.status_code == 200:
            print(f" {Fore.BLUE}<*> Successfully Joined Server {Fore.RED}({str(result.status_code)})")
        elif result.text.startswith('{"captcha_key"'):
            print(f" {Fore.RED}<!> Error Joining Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Captcha)")
        elif result.text.startswith('{"message": "401: Unauthorized'):
            print(f" {Fore.RED}<!> Error Joining Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Unauthorized)")
        elif "Cloudflare" in result.text:
            print(f" {Fore.RED}<!> Error Joining Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (CloudFlare Blocked)")
        elif "\"code\": 40007" in result.text:
            print(f" {Fore.RED}<!> Error Joining Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (User Banned)")
        elif "\"code\": 40002" in result.text:
            print(f" {Fore.RED}<!> Error Joining Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Locked Token)")
        elif "\"code\": 10006" in result.text:
            print(f" {Fore.RED}<!> Error Joining Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Invalid Invite)")
        else:
            print(f" {Fore.RED}<!> Error Joining Server {Fore.RED}({str(result.status_code)}){Fore.BLUE} ({result.text})")
...
def joiner():
    ...
    instances = []
    max_threads=5
    invite = input(f"{Fore.RED} <~> Invite: {Fore.BLUE}")
