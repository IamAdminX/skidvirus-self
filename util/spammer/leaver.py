...
class Leaveserver:
    ...
    def send(self) -> None:
        ...
        result=self.session.delete(f"https://discord.com/api/v9/users/@me/guilds/{self.instance.guild_id}",json={
            'session_id': self.rand_str(32),
        })

        if result.status_code == 204:
            print(f" {Fore.BLUE}<*> Successfully Left Server {Fore.RED}({str(result.status_code)})")
        elif result.text.startswith('{"captcha_key"'):
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Captcha)")
        elif result.text.startswith('{"message": "401: Unauthorized'):
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Unauthorized)")
        elif "Cloudflare" in result.text:
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (CloudFlare Blocked)")
        elif "\"code\": 40007" in result.text:
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (User Banned)")
        elif "\"code\": 40002" in result.text:
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Locked Token)")
        elif "\"code\": 10006" in result.text:
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Invalid Invite)")
        elif "\"code\": 10004" in result.text:
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Token Not In Server)")
        else:
            print(f" {Fore.RED}<!> Error Leaving Server {Fore.RED}({str(result.status_code)}){Fore.BLUE} ({result.text})")
