...
class Friender:
    ...
    def send(self) -> None:
        ...
        if result.status_code == 204:
            print(f" {Fore.BLUE}<*> Successfully Sent Friend Request! {Fore.RED}({str(result.status_code)})")
        elif result.text.startswith('{"captcha_key"'):
            print(f" {Fore.RED}<!> Error Sending Friend Request {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Captcha)")
        elif result.text.startswith('{"message": "401: Unauthorized'):
            print(f" {Fore.RED}<!> Error Sending Friend Request {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Unauthorized)")
        elif "Cloudflare" in result.text:
            print(f" {Fore.RED}<!> Error Sending Friend Request {Fore.BLUE}({str(result.status_code)}){Fore.RED} (CloudFlare Blocked)")
        elif "\"code\": 40007" in result.text:
            print(f" {Fore.RED}<!> Error Sending Friend Request {Fore.BLUE}({str(result.status_code)}){Fore.RED} (User Banned)")
        elif "\"code\": 40002" in result.text:
            print(f" {Fore.RED}<!> Error Sending Friend Request {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Locked Token)")
        elif "\"code\": 10006" in result.text:
            print(f" {Fore.RED}<!> Error Sending Friend Request {Fore.BLUE}({str(result.status_code)}){Fore.RED} (Invalid Invite)")
        else:
            print(f" {Fore.RED}<!> Error Sending Friend Request {Fore.RED}({str(result.status_code)}){Fore.BLUE} ({result.text})")
...
def userFriender():
    ...
    if not tokens:
        ...
        time.sleep(2)
        __import__("Xvirus").main2()

    instances = []
    max_threads=5
    user = input(f"{Fore.RED} <~> User USER#0000 (no need for # if the user has new username system): {Fore.BLUE}")
